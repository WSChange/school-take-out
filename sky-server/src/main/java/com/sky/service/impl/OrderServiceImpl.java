...
public class OrderServiceImpl implements OrderService {
    ...
    public OrderSubmitVO submitOrder(OrdersSubmitDTO ordersSubmitDTO) {
        ...
        // 查询当前用户购物车数据
        // 获取用户id,然后传入shoppingCart对象，最后在Mapper中传入该对象
     ...
     * @return
     */
    /*public PageResult pageQuery(OrdersPageQueryDTO ordersPageQueryDTO) {//select o.*,od.status from orders o left join orderdetails od on o.id = od.order_id where  order by order_time desc
        PageHelper.startPage(ordersPageQueryDTO.getPage(), ordersPageQueryDTO.getPageSize());
        Page<OrderOverViewVO> page = orderMapper.pageQuery(ordersPageQueryDTO);
...
//            )
   ...
   /* public PageResult conditionSearch(OrdersPageQueryDTO ordersPageQueryDTO) {
        ...
        // 获取订单号/手机号
        // 根据订单号/手机号进行搜索
        orderMapper.conditionSearch(orders);
    }*/
    ...
    public PageResult conditionSearch(OrdersPageQueryDTO ordersPageQueryDTO){
        PageHelper.startPage(ordersPageQueryDTO.getPage(),ordersPageQueryDTO.getPageSize());
        // 使用MyBatis-PageHelper插件的分页结果封装类，对Orders订单数据进行封装。
        Page<Orders> page = orderMapper.pageQuery(ordersPageQueryDTO);
...
//        return orderStatistics;
    ...
    public void confirm(OrdersConfirmDTO ordersConfirmDTO) {
        // 查询订单信息，然后判断订单状态，若为待接单2，则修改订单状态为已接单3
        // 直接用builder方法设置订单状态
        Orders orders = Orders.builder()
